...
const Profile = ({ data }: IProps) => {
  ...
  const [videoesList, setVideoesList] = useState<Video[]>([]);
  const [showUserVideos, setShowUserVideos] = useState(true);

  useEffect(() => {
    if (showUserVideos) {
      setVideoesList(userVideos);
    } else setVideoesList(userLikedVideos);
  ...
  return (
    <div className="w-full">
      <div className="flex gap-6 md:gap-10 mb-4 bg-white w-full">
        <div className="w-16 h-16 md:w-32 hd:h-32">
          <Image
            src={user.image}
            width={120}
            height={120}
      ...
      <div>
        ...
        <div className="flex gap-6 flex-wrap md:justify-start">
          {videoesList.length > 0 ? (
            videoesList.map((post: Video, index: number) => (
              <VideoCard post={post} key={index} />
            ))
          ) : (
            <NoResults
              text={`No ${showUserVideos ? "Posted" : "Liked"} videos yet`}
            />
          )}
        </div>
      </div>
